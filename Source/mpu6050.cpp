/*
 *	MPU6050 library ported for EasyMCU + some modification by Morteza Zandi @2016
 *	
 *	  ------> http://www.easymcu.ir
 *  
 *	Check out the links above for our tutorials and wiring diagrams
 *	Main Copyright texts as following ... 
 */
...
//			Activites state = readActivites();
	
//			while((getStatus() & 0x80) != 0x80);	// must replace with status register
//			while(state.isDataReady == 0);	// must replace with status register	
			i2cx->readLength = 6;
			i2cx->writeLength= 2;
			i2cx->masterBuffer[0] = mpuAddress|W_BIT;
			i2cx->masterBuffer[1] = MPU6050_REG_ACCEL_XOUT_H;	   //DATAX0
			i2cx->masterBuffer[2] = mpuAddress|R_BIT;
			i2cx->engine();
			x = (short)(( i2cx->slaveBuffer[0]<<8 ) | i2cx->slaveBuffer[1] ) ;
			y = (short)(( i2cx->slaveBuffer[2]<<8 ) | i2cx->slaveBuffer[3] ) ;
			z = (short)(( i2cx->slaveBuffer[4]<<8 ) | i2cx->slaveBuffer[5] ) ;
//			rawADXL[0] = (rawADXL[0] > 4095)? (rawADXL[0] & 0x0fff)-4096 : rawADXL[0];
//			rawADXL[1] = (rawADXL[1] > 4095)? (rawADXL[1] & 0x0fff)-4096 : rawADXL[1];
//			rawADXL[2] = (rawADXL[2] > 4095)? (rawADXL[2] & 0x0fff)-4096 : rawADXL[2];
	
			ra.XAxis = (float)x;
			ra.YAxis = (float)y;
			ra.ZAxis = (float)z;
			
//			ra.XAxis = (x > 1024)? (x & 0x03ff)-1024 : x;
//			ra.YAxis = (y > 1024)? (y & 0x03ff)-1024 : y;
//			ra.ZAxis = (z > 1024)? (z & 0x03ff)-1024 : z;	
...
//			while(state.isDataReady == 0);	// must replace with status register
			...
			i2cx->writeLength= 2;
			i2cx->masterBuffer[0] = mpuAddress|W_BIT;
			i2cx->masterBuffer[1] = MPU6050_REG_GYRO_XOUT_H;	   //DATAX0
			i2cx->masterBuffer[2] = mpuAddress|R_BIT;
			i2cx->engine();
			x = (short)(( i2cx->slaveBuffer[0]<<8 ) | i2cx->slaveBuffer[1] ) ;
			y = (short)(( i2cx->slaveBuffer[2]<<8 ) | i2cx->slaveBuffer[3] ) ;
...
//			rg.XAxis = (x > 1024)? (x & 0x03ff)-1024 : x;
//			rg.YAxis = (y > 1024)? (y & 0x03ff)-1024 : y;
//			rg.ZAxis = (z > 1024)? (z & 0x03ff)-1024 : z;	
...
//    uint8_t value;
	...
	return i2cx->read(mpuAddress, reg);
//    Wire.beginTransmission(mpuAddress);
//    #if ARDUINO >= 100
//	Wire.write(reg);
//    #else
...
{
	i2cx->write(mpuAddress, reg, value);
...
{
	return i2cx->read16(mpuAddress, reg, FIRST_MSB);
//    int16_t value;
//    Wire.beginTransmission(mpuAddress);
//    #if ARDUINO >= 100
//        Wire.write(reg);
...
{
	i2cx->write2(mpuAddress, reg, value);
//    Wire.beginTransmission(mpuAddress);
